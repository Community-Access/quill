...
def toggle_video(host: Any) -> None:
    ...
    window = VideoWindow(
        ...
        on_closed=lambda: _forget_window(host),
    )
...
def toggle_captions(host: Any) -> None:
    """**Captions** (Ctrl+Shift+C on the video surface). On, off, and honest."""
    engine = _engine(host)
    if engine is None:
        host._announce(NEEDS_MPV)
        return
    ...
    if not url:
        ...
        return
    wanted = not bool(getattr(host, "_captions_on", False))
    host._captions_on = wanted
    if wanted:
        engine.add_subtitles(url)
        _apply_captions(host, engine, first_time=False)
        # Said once, and said plainly. An automatic caption presented as
        # authoritative is worse than one presented honestly.
        host._announce(
            "Captions on. These are automatic captions, so expect mistakes."
            if automatic
            else "Captions on."
        )
    else:
