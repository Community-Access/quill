"""Quick Actions... -- put the actions you use at the top. Shared by both apps.

The reorder pattern is the one QUILL uses everywhere a list is rearranged by
keyboard: a real ``wx.ListBox`` plus Move Up / Move Down / Make Default buttons,
never checkboxes inside a list control. A checkbox in a list is a state a screen
...
One list per context, chosen with a combobox above it, so the whole dialog is
one tab stop deeper rather than several panels wide.

**Nothing in here knows what an episode or a station is.** The catalogue, the
labels and the store all arrive as arguments, so Cast and Radio share one dialog
rather than two that drift -- which is what happened to the transcript reader and
to the search-within-a-folder knowledge before somebody noticed.
"""
...
from quill.core.quick_actions import DIRECT_KEY_COUNT, QuickAction, QuickActionOrders
...
class QuickActionsDialog:
    ...
    def __init__(
        ...
        *,
        ...
        self._context = self._labels[0][0] if self._labels else ""
        ...
        self._context_choice = wx.Choice(
            self.dialog, choices=[label for _cid, label in self._labels]
        )
    ...
    def _actions(self) -> list[QuickAction]:
    ...
    def _on_context_choice(self, _event: object) -> None:
        ...
        self._context = self._labels[index][0]
        self._refill()
        self._announce(f"{self._labels[index][1]}: {len(self._actions())} actions")
    ...
    def _reset(self) -> None:
        ...
        self._refill()
        label = dict(self._labels).get(self._context, "This list")
    ...
    def _on_ok(self, _event: object) -> None:
        ...
        # order, but the record is about to be written to disk and read back by
        # a future build, and that is the file that has to be trustworthy.
        for context, _label in self._labels:
