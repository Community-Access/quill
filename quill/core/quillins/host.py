    ...
    def _invoke_service(self, method: str, args: list[Any]) -> Any:
        ...
        if method == "announce":
            if not services.is_verbosity_speech_enabled():
                return None
            # #1306: an optional severity so an extension can signal a
            # problem properly instead of every message reading as "Saved".
            # Defaults to info, so no existing Quillin changes behaviour.
            severity = str(args[1]) if len(args) > 1 else "info"
            try:
                services.announce(str(args[0]), severity)
            except TypeError:
                # A host that predates the severity argument.
