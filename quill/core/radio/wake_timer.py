...
One configured wake-up (once or daily), persisted as atomic JSON, checked
by a lightweight background thread while the app runs. Deliberately honest
about its limits: QUILL (or standalone Quill Radio) must be running -- in
the tray is fine -- for the wake-up to fire; nothing is installed into the
OS task scheduler. wx-free, strict-typed.
"""
...
class WakeUpWatcher:
    ...
    themselves (the same contract as the recorder and scheduler)."""
