...
search results alongside RadioBrowser -- one more reputable, no-API-key,
no-commercial-terms source, in the same spirit as the bundled ACB Media
category (``core/radio/acb_media.py``). Unlike ACB Media's static list,
SomaFM's ~30-channel lineup is fetched live (it changes occasionally and
SomaFM explicitly publishes this feed for third-party players), but it is
folded into the *same* search results list as RadioBrowser -- no separate
menu, no visible "from SomaFM" label -- so it reads as one directory to the
user, not two. See PRD §5.84f.

Two egress calls, both reviewed (see ``quill/tools/network_egress_audit.py``):
the channel list itself (``_http_json``), and -- only for channels that
match the user's search -- resolving that channel's ``.pls`` playlist to a
real playable stream URL (``_http_text``), since SomaFM's own JSON only
ever links to playlist manifests, never a raw stream. Both HTTPS-only with a
verified TLS context, disabled in Safe Mode via :func:`refuse_in_safe_mode`.
