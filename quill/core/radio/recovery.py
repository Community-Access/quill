"""Self-healing recovery for a station whose stream won't play (#1065).

A directory station's listed stream can be dead even though the station is
...
web-player address, not a stream. Gene's report of Magic 104.1 / KMGL is
exactly this: found in the directory, wouldn't play, and the working stream
was only reachable by digging through the station's website by hand.
...
* **Strategy A -- re-resolve a StreamTheWorld mount (high confidence, always).**
  ...
  :mod:`quill.core.radio.triton`) to its current server. Deterministic, same
  station and provider -- no guessing.
...
from quill.core.radio.link_finder import LinkFinderError, PageStreamCandidate
