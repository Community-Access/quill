...
Many broadcast stations (thousands of US AM/FM stations, and the whole
...
:mod:`quill.core.radio.link_finder` (which deliberately never runs JavaScript)
finds nothing on those pages, even though a Play button is plainly visible.
...
The single egress call funnels through :func:`_fetch_api` (reviewed in
``quill/tools/network_egress_audit.py``), HTTPS-only over a verified TLS
context with a bounded timeout, reached only by the same explicit "Scan"
button as the rest of :mod:`link_finder` and disabled in Safe Mode via
...
def refuse_in_safe_mode(safe_mode: bool) -> None:
    ...
    Safe Mode (``QUILL_SAFE_MODE=1``) disables every network service, and
    resolving a Triton player is one. Kept in core (flag passed in) so the
    refusal is unit-testable without wx, mirroring the sibling radio sources.
    """
    if safe_mode:
...
def page_is_triton_player(url: str, html: str) -> bool:
    ...
    fetched. Used to gate the extra provisioning-API round trip so QUILL only
    ever contacts Triton for pages that actually are Triton players.
    """
    haystack = f"{url}\n{html}".lower()
