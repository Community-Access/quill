...
- **`menu_lint` Ctrl+Alt+ gate no longer over-matches Ctrl+Alt+Shift+.** The §10.8 gate's regex was a prefix match, so every `Ctrl+Alt+Shift+<letter>` binding was flagged as a §10.8 violation even though modifier-stacking is governed by §10.4. The regex now uses a negative lookahead `(?!shift\+)` so `Ctrl+Alt+<letter>` is the only pattern this gate fires on. Two new tests cover the regression and the lowercase form; `test_run_checks_passes_on_current_codebase` continues to pass on the live keymap (#347).
