...
def test_authorize_url_targets_oauth_with_read_and_write_scopes() -> None:
    ...
    # read + write:statuses + write:follows + write:lists (read/interact surface).
    assert "scope=read+write%3Astatuses+write%3Afollows+write%3Alists" in url
...
def test_register_app_posts_quill_client_name(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert calls[0]["data"]["scopes"] == "read write:statuses write:follows write:lists"
