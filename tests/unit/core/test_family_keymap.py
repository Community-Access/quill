...
So: every QUILL Lite handler maps to the QUILL command id that means the same
...
**The table is the review.** A new QUILL Lite command fails this until somebody
...
def test_every_quilllite_command_is_accounted_for() -> None:
    ...
    assert unaccounted == [], (
        "QUILL Lite handlers with no row in quill/core/lite/parity.py: "
        + ", ".join(unaccounted)
...
def test_the_two_editors_agree_about_this_chord(handler: str, command_id: str) -> None:
    ...
    assert _same_chord(lite_key, command_id), (
        f"{handler} is {lite_key!r} in QUILL Lite and "
...
def test_no_chord_means_two_different_things_across_the_editors() -> None:
    ...
    Ctrl+Alt+J was Set Temporary Bookmark in QUILL and Justify in QUILL Lite;
    ...
    for handler, lite_key in sorted(lite.items()):
        ...
        for command_id, quill_key in DEFAULT_KEYMAP.items():
            ...
            if handler in NATIVE_IN_BOTH or handler in LITE_ONLY:
                continue
            offenders.append(f"{lite_key}: {handler} in QUILL Lite, {command_id} in QUILL")
    assert offenders == [], "one chord, two meanings:\n  " + "\n  ".join(offenders)


def test_the_gap_list_is_a_backlog_not_a_shrug() -> None:
    """Everything in LITE_ONLY is QUILL Lite ahead of QUILL, which rule 10 forbids.
    ...
    assert len(LITE_ONLY) <= 8, (
        "QUILL Lite is ahead of QUILL in more places than the plan accounts for"
    )
