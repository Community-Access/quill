...
def _load_build_portable():
    spec = importlib.util.spec_from_file_location(
        "quill_build_portable",
        _ROOT / "standalone" / "studio" / "scripts" / "build_portable.py",
    )
    assert spec and spec.loader
    module = importlib.util.module_from_spec(spec)
    # Register before exec: build_portable defines a @dataclass, and dataclasses
    # resolves its string annotations through sys.modules[cls.__module__]. An
    # unregistered module makes that lookup raise AttributeError at class-creation
    # time, so the import fails before any assertion runs.
