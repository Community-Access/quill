@echo off
rem QuillLite, from this checkout rather than from an installed build.
rem
...
rem
rem QuillLite keeps its state in QUILL_LITE_DATA_DIR, not QUILL_DATA_DIR --
...
rem QuillLite install. Point QUILL_LITE_DATA_DIR at %LOCALAPPDATA%\QuillLite
