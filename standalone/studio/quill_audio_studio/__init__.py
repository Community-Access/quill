"""QUILL Audio Studio standalone launcher.

The application itself lives in the ``quill`` package (``quill.apps.studio``)
...
def _export_app_root() -> None:
    ...
    - Portable mode: the portable zip ships a ``data`` folder next to
      QuillAudioStudio.exe. When it's there, export QUILL_APP_ROOT and
...
def main() -> int:
    _export_app_root()
    from quill.apps.studio import main as run
