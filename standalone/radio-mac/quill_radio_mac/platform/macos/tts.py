...
- **Voice catalog** (:func:`list_voices`): the system voice list from
  ...
  the ``say`` CLI, so a read-aloud runner can shell out to ``say`` -- safe on a
  worker thread where instantiating an AppKit object off the main thread would
  be unsafe.

pyobjc is imported lazily inside each function (mirroring
``quill_radio_mac.platform.macos.announce``) so the module imports cleanly on
non-macOS / pyobjc-less machines and every function degrades to ``False`` /
``[]`` / no-op. Ported verbatim from ``quill.platform.macos.tts``; only this
docstring was expanded for the port.

Threading contract: :func:`speak_announcement` and :func:`stop_announcement`
are meant to be called from the UI thread alongside
:func:`quill_radio_mac.platform.macos.announce.announce`; they do not spawn
threads of their own. :func:`list_voices` and :func:`available` are safe to
call from any thread (each pyobjc call they make is a synchronous query, not
a UI mutation).
"""
...
def speak_announcement(text: str, *, voice: str | None = None, rate: int | None = None) -> bool:
    ...
    Returns ``False`` when *text* is empty or pyobjc / AppKit is unavailable
    (including on any non-macOS platform).
    """
    if not text:
        return False
    try:
