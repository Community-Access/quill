...
Ported near-verbatim from upstream ``quill.core.safe_xml``. The stdlib
:mod:`xml.etree.ElementTree` parser still expands internal entities,
which leaves it open to "billion laughs" denial-of-service attacks
from crafted documents. This module provides a single ``fromstring``
entry point that disables entity expansion and external-entity
resolution, used by :mod:`quill_radio_mac.core.triton` to parse the
provisioning API's ``<live_stream_config>`` XML -- untrusted network
input that must never be handed to the raw stdlib parser.

When :mod:`defusedxml` is available (optional; this app adds no new
required runtime dependency) it is used directly. If it is not
installed, a conservative fallback rejects any document that declares
a DTD or custom entities before falling back to the stdlib parser, so
an attack payload is refused rather than expanded.

Threading contract: pure functions over stdlib/defusedxml parsers,
which are safe to call from any thread; :mod:`quill_radio_mac.core.triton`
calls this off the UI thread while resolving a station's stream.
...
def _as_bytes(text: str | bytes) -> bytes:
    """Encode *text* to bytes for the DTD/entity pre-scan (pure)."""
