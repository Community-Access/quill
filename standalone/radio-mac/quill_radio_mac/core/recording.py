...
A radio stream is indefinite -- unlike a podcast episode, there is no
natural end -- so recording needs a controllable start/stop rather than
a single blocking call. This launches ``ffmpeg`` via
:class:`subprocess.Popen` (with the same ``CREATE_NO_WINDOW`` / logged-
args safety properties as upstream's ``run_subprocess_safely``, which
cannot be used here because it blocks until the process exits) and stops
it by writing the ``q`` keypress ffmpeg's own stdin-driven quit handler
reads -- the same graceful stop a person pressing "q" in a terminal
gets, closing the output file's container properly instead of a hard
kill truncating it.

Recording reuses the app's one ffmpeg-discovery module
(``quill_radio_mac.core.ffmpeg``) -- the same one the recordings-folder
codec probe depends on -- rather than introducing a second ffmpeg
dependency path.

Ported near-verbatim from upstream ``quill.core.radio.recording``: the
``Popen``/stdin-``q`` stop, the reconnect "part N" continuation-file
logic, and the ``os.name == "nt"`` ``CREATE_NO_WINDOW`` guard are all
unchanged (the guard is a harmless no-op on macOS, kept to minimize the
diff against upstream and because the core test suite also runs on
Windows).

wx-free, strict-typed.

Threading contract: :class:`RadioRecorder` is safe to use from any
thread. ``start``/``stop`` and the internal state are guarded by a lock;
``on_state_changed`` and ``on_reconnect`` fire on the recorder's own
background monitor thread (named ``quill-radio-record-monitor``), so a
UI caller must marshal back to the main thread itself (the same contract
the task manager, scheduler, and wake timer use).

macOS notes: none beyond the process-management code itself, which is
fully cross-platform; the only platform-specific piece is the harmless
``CREATE_NO_WINDOW`` guard above.
"""
...
from quill_radio_mac.core.ffmpeg import INSTALL_HINT, find_ffmpeg, find_ffprobe
from quill_radio_mac.core.redaction import format_args_for_log
...
#: ffmpeg stream-copies the server's audio packets with no re-encoding,
#: so the saved file is bit-for-bit the original stream -- the most
#: lossless capture possible, for a listener who wants to edit/convert it
#: themselves. All are streamable containers (no trailing index atom like
#: MP4/M4A), so even an unclean stop leaves a playable file up to the
#: last flushed frame.
...
#: The raw-capture container extension for a probed source codec. Common
#: codecs get their natural elementary-stream extension so the file opens
#: anywhere; anything else falls back to Matroska audio (``.mka``), which
#: stream-copies any codec losslessly into one file the user can extract
#: from later.
...
class RecordingSettings:
    ...
    #: copy) even for a listener with Sound Enhancements on. When true, a
    #: new recording is filtered through the *current* EQ preset/compressor
...
def _sanitize_filename_component(text: str) -> str:
    """Strip characters that are invalid in a filename on any of this
    app's supported platforms, collapsing whitespace runs to one space."""
...
def build_record_command(
    ...
    *,
    ...
    Pure and unit-tested. ``-t`` caps every recording at
    ``duration_seconds`` even if :meth:`RadioRecorder.stop` is never
    called, so a forgotten recording cannot grow unbounded. A positive
    ``reconnect_delay_max`` turns on ffmpeg's own HTTP reconnect handling
    (first line of defense against a dropped connection; the recorder's
    process-level retry is the second), valid only for http(s) inputs. A
    non-empty ``filter_graph`` (built by
    ``core.audio_enhance.build_filter_graph``, this module stays
    decoupled from that one) records the *filtered* audio -- Sound
    Enhancements applied to the archival copy, not just live playback.

    ``format="copy"`` is the raw-capture mode: ffmpeg stream-copies the
    server's audio packets (``-c:a copy``) with no decode/re-encode, so
    the file is bit-for-bit the original. A bitrate and a filter graph
    are meaningless with no re-encode and are ignored (Sound Enhancements
    cannot apply to a raw copy). The output container is chosen by
    ``out_path``'s extension, which the caller resolves from the
    stream's own codec.
    """
    ...
    if format == "copy":
        # Raw capture: copy packets verbatim -- no filter (nothing is
        # decoded), no bitrate (no re-encode).
...
def raw_capture_extension(codec: str) -> str:
    ...
    Common codecs get their natural extension; anything else falls back
    to Matroska audio (``.mka``), which stream-copies any codec
    losslessly.
    """
...
class RadioRecorder:
    ...
    ``on_state_changed(is_recording, destination)`` fires on a
    background thread when a recording starts or ends (naturally, via
    :meth:`stop`, or by hitting its duration cap) -- callers that touch
    wx must marshal back to the UI thread themselves, the same contract
    this app's other background workers use.
    """
    ...
    def __init__(
        ...
        *,
        ...
        #: (attempt, max_attempts) -- fired on a background thread each
        #: time a dropped recording is about to be resumed into a
        #: continuation file.
        ...
        #: (station_name, stream_url, settings, duration_minutes,
        #: filter_graph, resolved_extension) of the active recording,
        #: kept so a reconnect can restart with the same shape --
        #: including any Sound Enhancements filter and, for raw-capture
        #: mode, the extension resolved on the first probe so a
        #: continuation file never re-probes or changes container.
    ...
    def start(
        ...
        *,
        ...
        Raw-capture mode (``settings.format == "copy"``) ignores the filter
        and writes a container matching the stream's own codec."""
        with self._lock:
            ...
            # Raw capture: the file extension follows the server's own
            # codec (probed once, reused across continuations); a filter
            # is impossible without decoding, so it is dropped. Re-encode
            # formats keep their format name as the extension and honor
            # the filter.
...
def _probe_capture_extension(stream_url: str) -> str:
    ...
    Best-effort: a missing ffprobe, a probe error, or a timeout all fall
    back to Matroska audio (``.mka``), the universal lossless copy
    container, so raw capture always has a valid destination.
    """
...
def _default_dir() -> Path:
    # Mirrors upstream's own ``app_data_dir() / "radio_recordings"``
    # literal rather than calling ``paths.recordings_dir()``: that
    # helper eagerly creates the folder on every call, which would make
    # even a read-only settings/listing lookup materialize an empty
    # ``radio_recordings`` directory. Here the folder is created only
    # when a recording actually starts (see ``RadioRecorder.start``).
