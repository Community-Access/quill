...
Ported near-verbatim from upstream ``quill.core.radio.favorites``. No
sibling ``core/publish/destinations.py`` exists in this standalone
port, so that upstream cross-reference is dropped; everything else --
fields, methods, load/save behavior -- is unchanged.
...
Threading contract: plain dataclasses and pure functions except
:func:`load_favorites`/:func:`save_favorites`, which do blocking file
IO and are meant to be called from a worker thread (the task manager),
never the UI thread.

macOS notes: none -- fully platform-neutral. ``radio_favorites.json``
lives under ``app_data_dir()`` (``~/Library/Application Support/Quill``
on macOS), matching the Windows app's file name and schema so a data
folder copied from a Windows machine loads unchanged.
...
from quill_radio_mac.core.audio_enhance import clamp_eq_gain
...
class FavoriteStation:
    ...
    #: has_sound_enhancement_override is True. While False the fields below
    #: are unused; the station follows RadioHistory's shared
    #: eq_bass_db/mid/treble/compressor_enabled.
...
class RadioFavoritesStore:
    ...
    def set_enhancement(
        ...
        *,
        ...
        shared default (RadioHistory.eq_bass_db/mid/treble_db/compressor_enabled)
...
def load_favorites(data_dir: Path) -> RadioFavoritesStore:
    ...
    if isinstance(raw, dict):
        ...
        for folder_path in raw_folders if isinstance(raw_folders, list) else []:
            if isinstance(folder_path, str) and folder_path.strip():
                store.folders.append(folder_path.strip().strip("/"))
