...
favorites store, and every UI surface (station browser, status bar, Dock
menu).

Ported verbatim from upstream ``quill.core.radio.models``. wx-free,
strict-typed. The ``to_dict``/``from_dict`` schema is the on-disk format
of ``radio_favorites.json`` and the recents list in
``radio_history.json``; it must stay byte-compatible with the Windows
app so a copied data folder round-trips without loss.

Threading contract: plain immutable-ish dataclass and pure functions;
instances are created on worker threads (directory searches) and read on
the UI thread without locking, which is safe because nothing mutates a
station after construction.
...
class RadioStation:
    ...
    def to_dict(self) -> dict[str, object]:
        """Serialize for JSON persistence (``tags`` becomes a list)."""
        return {
            "name": self.name,
    ...
    def from_dict(cls, data: dict[str, object]) -> RadioStation | None:
        """Rebuild a station from a persisted dict.

        Lenient by design: numeric fields go through :func:`_coerce_int`,
        a non-list ``tags`` value is dropped, and a record missing its
        ``name`` or ``stream_url`` returns ``None`` so one corrupt entry
        never breaks loading the rest of the favorites file.
        """
