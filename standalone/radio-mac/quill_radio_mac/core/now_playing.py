...
Ported near-verbatim from upstream ``quill.core.radio.now_playing``. A
stream's ICY ``StreamTitle`` (see :mod:`quill_radio_mac.core.icy`) is
supposed to be a short "Artist - Title" string, and for most stations it
is. But broadcast automation systems (RCS/GSelector, iHeart, and other US
commercial stations) stuff a pile of key/value pairs in there instead, so
a listener who asks "what's playing?" hears the raw mess::
...
with the announcing UI (a later task); this module owns only the track
description itself.

Threading contract: pure functions and dataclasses; safe from any thread.
...
def parse_now_playing(stream_title: str) -> NowPlaying:
    ...
    Recognises, in order: the ``key="value"`` broadcast-automation form, the
    plain ``Artist - Title`` convention, and -- failing both -- a bare string
    taken as the title. Never raises; an empty input yields an empty
    :class:`NowPlaying`.
    """
