...
(``quill_beacon.sync.FolderTransport``) is the no-account tier.
...
class ServerClient:
    ...
    def request_magic_link(base_url: str, email: str, *, session=None) -> dict:
        ...
        r = s.post(f"{base_url.rstrip('/')}/auth/request",
                   json={"email": email}, timeout=10)
        r.raise_for_status()
        return r.json()

    @staticmethod
    def verify_magic_link(base_url: str, token: str, device: str = "device",
                          *, session=None) -> dict:
        s = session or requests
        r = s.get(f"{base_url.rstrip('/')}/auth/verify",
                  params={"token": token, "device": device}, timeout=10)
        r.raise_for_status()
        return r.json()
    ...
    def push(self, commits: list[dict], objects: dict[str, bytes]) -> dict:
        ...
        r = self.session.post(f"{self.base_url}/sync/push", json=body,
    ...
    def pull(self, have: list[str]) -> dict[str, Any]:
        ...
        r = self.session.post(f"{self.base_url}/sync/pull", json={"have": have},
                              headers=self._headers(), timeout=30)
        r.raise_for_status()
        data = r.json()
    ...
    def hints(self) -> dict:
        r = self.session.get(f"{self.base_url}/sync/hints",
