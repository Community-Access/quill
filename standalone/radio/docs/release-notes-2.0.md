...
- **"Radio Browser," spelled out -- and browsable by genre.** The Search source picker now says **Radio Browser** in two words (a screen reader could make the old run-together spelling sound like the option was gone), and Browse Stations has a new **Radio Browser (by Genre)** branch so you can wander that huge directory by genre instead of only searching it.
