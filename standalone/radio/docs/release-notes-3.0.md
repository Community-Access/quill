...
> large -- a body of work that was finished but never put in anybody's hands
> arrives with it. The second half, **Everything else that arrives with 3.0**, is
> as new to you as the first.
...
## Everything else that arrives with 3.0

The work in this half was finished and documented, and then never put in
anybody's hands. Rather than leave it in a release note nobody would think to
open, all of it is here: if you are coming from 2.1.2 -- and everyone is -- this
half is as new to you as the half above.

This is the half where the app grows up around you. Windows stop
...
**The recordings list has no play queue**, which is why the Winamp map leaves
shuffle, repeat and stop-after-current out. A key that only looks like it worked
