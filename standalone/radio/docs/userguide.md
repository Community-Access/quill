...
Shuffle, repeat, and stop-after-current are not here yet: all three describe a play queue, which this list does not have. They are on the list for a later release rather than bound to a key that would only look like it worked.
