...
The keys below are the ones Quill Radio ships with. If you have rebound something in the Keyboard Manager, the tutorials *inside the app* say your key; this document cannot know it.

## Contents

- **Your first hour** -- Start here. By the end of this track you can find a station, keep it, work the player from any window, and get yourself unstuck without asking anybody.
  - Play your first station (5 minutes)
  - Keep a station, and find it tomorrow (4 minutes)
  - The player follows you (6 minutes)
...
- **Finding something to listen to** -- Several ways in: the tree, the search across every directory at once, addresses of your own, and the catalog on your own disk that answers when the internet does not.
  ...
  - When a station will not play (6 minutes)
...
- **Recording** -- From one keypress to a show that records itself every Tuesday while you are out -- and what happens when the connection does not hold.
  - Record what is on now (6 minutes)
  - Book a show that has not started yet (8 minutes)
  - Record several stations at once (6 minutes)
  - Live in the Recordings list (7 minutes)
  - When a recording breaks (6 minutes)
- **More than radio** -- Podcasts, audiobooks, YouTube, television and the ACB Media schedule all arrive through the same tree and play with the same keys.
  - Follow a podcast (8 minutes)
  - Audiobooks, archives and free music (8 minutes)
  - YouTube, with no account anywhere (7 minutes)
...
- **Living with it** -- The parts you meet after the first week: what was that song, keeping a moment, sleeping, statistics, and where to look when something goes wrong.
  - What was that song? (5 minutes)
  - Keep a moment, and move around inside one (6 minutes)
  - Sleeping, waking, and being left alone (6 minutes)
...
Start here. By the end of this track you can find a station, keep it, work the player from any window, and get yourself unstuck without asking anybody.

### Play your first station

Open the browse tree, find a station that is on the air right now, and hear it. This is the loop the whole app is built on: arrow to a thing, press Enter.
...
Next: Keep a station, and find it tomorrow.

### Keep a station, and find it tomorrow

Turn the station you are listening to into a favorite, then reduce getting back to it to two keystrokes: launch, Enter.
...
2. **Find it in the tree.** Move focus to the favorites tree in the main window and arrow down. Your station is there. This tree is the main window's whole purpose: it is a list you play from, not a second copy of the player.
   - Keys: Down arrow
   - You should hear: The station's name, in the favorites tree.
...
Next: The player follows you; Do anything by name.

### The player follows you

Learn the handful of keys that work in every window, and the window that holds the whole player. This is the part of Quill Radio that is unlike other radio programs, and the part worth ten minutes.
...
6. **Ask where you are.** Press Where Am I. On a recording or an episode it tells you the position, the length and the chapter. On live radio it tells you that this is live radio, which plays at broadcast speed and has no position to move through -- a refusal with a reason, rather than a key that quietly does nothing.
   - Keys: Ctrl+Shift+W
   - You should hear: Either a position, or the sentence explaining why a live stream has none.
...
Three ways to reach anything without remembering a key: the command palette, the numbered list of places, and the sheet that lists every key you actually have.

*6 steps, about 4 minutes.*

1. **Open the command palette.** The palette opens from every window and lists every command in the app, including the whole player -- so it can pause what is playing, not just change a setting.
   - Keys: Ctrl+Shift+P
   - You should hear: A search box, with the number of commands available.

2. **Type what you want, not what it is called.** Type a few letters -- vol, or record, or chapter -- and the list narrows as you type. Arrow to the one you want and press Enter; it runs exactly as its key or its menu item would.
   - Keys: Down arrow, Enter
   - You should hear: The matching commands, each read with its own keystroke.
   - Worth knowing: Each entry shows its key, so the palette teaches you the shortcut while you use it. That is deliberate: the palette is meant to make itself less necessary.

3. **Open the list of places.** Go To is a short numbered list of the ten places in the app. Press the number and you are there; Escape puts you back exactly where you were, on the same control.
   - Keys: Ctrl+G
   - You should hear: A numbered list, starting with Favorites.

4. **Understand why the numbers are worth learning.** A place's number never changes on its own. Recordings is 4 today and 4 next year, whether or not it is open. That is what Ctrl+1 to Ctrl+9 cannot promise -- those reach the windows you have open, in the order you opened them, so the numbering shifts under you all day.
   - Keys: Escape
   - You should hear: Nothing new: this step is a fact, not an action.

5. **Open the sheet of every key.** The Keyboard Shortcuts Sheet lists every key the app answers to, filterable. Type what you want to do -- record -- or a key you found and cannot place -- Ctrl+B -- and the list narrows to it.
   - Keys: Ctrl+Alt+Shift+K
   - You should hear: A filter box, then the number of shortcuts listed.
   - Worth knowing: The sheet is built by reading the menu bar in front of you, so it shows the keys you actually have. Rebind something and the sheet says your key, not the default.

6. **Ask what the thing under your fingers is.** Press F1 anywhere. A window opens with two parts read as one pass: what the window you are in is for, then what the control under focus does and how to drive it. The text sits in a field you can arrow through and copy, and Escape returns you exactly where you were.
   - Keys: F1
   - You should hear: The window's purpose, then the control's own help.

Between the palette, Go To, the sheet and F1, there is no state of this app you can be in and not have a way out of. That is the point of all four.

Next: Getting unstuck.

### Getting unstuck

The short list to reach for when something is not where you expected, you missed what was said, or a menu item will not press. Read it once now so it is familiar when you need it.
...
Several ways in: the tree, the search across every directory at once, addresses of your own, and the catalog on your own disk that answers when the internet does not.

### Wander the browse tree

Learn the tree well enough to explore it: how a branch loads, what a row is telling you before you press Enter, how to search inside one folder, and how to put a source's own options where you want them.
...
Next: Search every directory at once; Find stations by name, tag and country.

### Search every directory at once

Run one search across every source you have switched on, understand what the answer is telling you, and know why a short list is honest rather than complete.
...
Use the field-based search window, get back to a search you ran before in one keystroke, and decide which directories are asked.
...
2. **Search the catalog first, and the internet second.** Type a name and press Enter. Matches from the catalog on your own disk appear the moment you search; the live directories layer in behind them. That is why the first results arrive instantly even on a slow connection.
   - Keys: Enter
   - You should hear: A count of results, then the list.

3. **Bring back a search you already ran.** Press Down arrow in the station-name box for the searches you ran before, newest first. Picking one restores all three fields together, because they were one search; a list that kept only the word jazz would give you back the wrong one.
   - Keys: Down arrow
...
Next: The catalog on your own disk; When a station will not play.

### The catalog on your own disk

Understand why browsing is instant and works with no internet at all, how fresh your copy is, what is deliberately not in it, and how to update or rebuild it.

*6 steps, about 5 minutes.*

1. **Notice what is answering.** Open By Country, By Language, By Genre or By Quality and time it. They answer from your own disk in under a millisecond, because Quill Radio ships the whole working-station directory inside the app -- more than 62,000 stations across 240 countries -- and keeps it in a catalog on this computer.
   - Keys: Ctrl+B
...
Read what the status line is actually telling you, let Quill Radio try to repair a dead address, check whether this installation can play at all, and report a station that is genuinely gone.
...
2. **Let it repair the address.** Some stations are listed with an address that has since died. Instead of failing, Quill Radio works down a ladder: it re-resolves a moved player address, refreshes the address from the directory, and -- with the setting on -- scans the station's own website for its Listen Live player. One clear stream is played and remembered for that favorite.
   ...
   - Worth knowing: The website step is Recover failed streams from the station's website in Preferences, on by default and off in Safe Mode. It only tries once per station per session.

3. **Check whether the problem is this installation.** Audio Health answers is this going to work in one list: which engine is actually in use, whether mpv and FFmpeg are present and what their absence costs, where the audio is going, and whether a recording could be written right now. It tests nothing -- no sound played, no device opened -- so it is safe to open mid-recording.
...
6. **Look up what you missed.** If the failure was spoken while you were in another window, Recent Problems still has it, with the reason and the time. Retry tries the highlighted row again, and Copy All takes the list as text for a report.
   - Keys: Ctrl+Alt+Shift+P
   - You should hear: The failures, newest first, each with its reason.

Most failures here are streams moving, not the app breaking. The ladder above catches most of them before you notice; the report catches the ones nobody else can see.

## Making it yours

Folders, order, columns, keys, and the handful of settings that change how the app feels rather than what it can do.

### Folders, and an order of your own

Build folders of any depth, move stations around without hunting, and play a whole folder as a set. Do this while you have twenty favorites rather than two hundred.
...
Next: Ten stations, no list at all.

### Ten stations, no list at all

Put your ten most-played stations on ten chords, learn the two routes back to what you played recently, and decide what the main window shows in the first place.
...
Next: Make the keys yours.

### Make the keys yours

Rebind anything, give the player keys that work from inside other programs, and know which keys are already system-wide before you start.
...
Every tutorial in this set names commands rather than keys, so once you have rebound something the lessons say your key too.

Next: The settings actually worth changing.

### The settings actually worth changing

A guided pass through Preferences, stopping only at the settings that change something you will notice. Everything else can stay as it is.
...
### Quillins: extensions in a radio

What a Quillin is, where they appear in Quill Radio, and what one can contribute to the browse tree.
...
2. **See what one can contribute.** A Quillin can contribute a whole station source, not only search results. When one is installed and enabled, a Quillin Sources branch appears in Browse Stations -- one folder per contributed source, with its categories and its stations, playable and favouritable like anything else.
   - Keys: Ctrl+B
   - You should hear: Quillin Sources, and the contributed source beneath it.

3. **Notice when there is nothing to notice.** With no Quillin contributing a source, that branch is simply absent rather than present and empty. An empty branch is a question you have to answer; an absent one is not.
   - You should hear: Nothing: this is the branch you will not find.
...
From one keypress to a show that records itself every Tuesday while you are out -- and what happens when the connection does not hold.

### Record what is on now

Start and stop a recording of the station you are listening to, find the file, play it back, and understand what the status bar's Record cell is counting.
...
Next: Book a show that has not started yet.

### Book a show that has not started yet

Fill in a scheduled recording correctly the first time, including the time-zone trap, then edit, duplicate, disable and delete entries without starting again.

*10 steps, about 8 minutes.*
...
Next: Record several stations at once; When a recording breaks.

### Record several stations at once

Capture a station you are not listening to, run several captures side by side, stop them individually or all together, and keep a bit-for-bit copy when quality matters.
...
Next: Live in the Recordings list.

### Live in the Recordings list

The Recordings window is a playlist editor by any other name, and it has Winamp's classic-skin keys on the letter keys you already know. Learn the dozen that matter.
...
3. **Move through the list.** B moves down the list and plays; Z goes back. Up and Down arrow move through the list without playing, which is what Winamp's own Playlist Editor does -- volume stays on Ctrl+Up and Ctrl+Down, where it has always been.
   - Keys: B, Z
   - You should hear: The recording that started, named.

4. **Seek without leaving the row.** Left and Right move five seconds; Shift with them moves thirty. Seeking needs a recording with a timeline, which means the mpv engine -- on a live stream or the classic engine the keys say why they cannot move rather than doing nothing.
   - Keys: Left, Right, Shift+Left, Shift+Right
   - You should hear: The new position, or the reason it cannot move.

5. **Ask the time, and jump to one.** T reads the elapsed time; press it again and it reads the time remaining instead. Ctrl+J jumps to a time you type -- 90, 1:30, or 1:02:03 all work -- and J jumps to a recording by any part of its name.
   - Keys: T, J, Ctrl+J
   - You should hear: The time, or the recording you jumped to.
   - Worth knowing: Ctrl+T stays What's Playing here, which is the more useful thing to have on that key in a radio app. Winamp's elapsed/remaining toggle is on plain T instead.

6. **Shuffle, repeat, and stop after this one.** R turns shuffle on and off, S cycles repeat -- off, all recordings, this recording -- and Ctrl+V stops after the current recording. Shuffle is a fixed order rather than a fresh roll each time, so every recording plays once before any repeats and Z reliably takes you back.
   - Keys: R, S, Ctrl+V
...
Next: When a recording breaks.

### When a recording breaks

What Quill Radio does about a dropped connection, a stalled stream, a crash mid-capture and a part file -- and the handful of settings that decide how hard it tries.
...
2. **Know what a drop actually costs.** A truly dead connection is resumed into a numbered part file, with each attempt announced. When the recording finishes, the parts are stitched back into one file under the name you expected -- so a show that dropped twice does not leave you three files to play in order.
   ...
   - Worth knowing: The join is a straight copy, so nothing is re-encoded and even a three-hour capture takes seconds. The parts are removed only after the joined file is written and checked; a failed join never costs you the recording.

3. **Understand a stall, which is not a drop.** A connection can go quiet without disconnecting -- a pulled cable, a dropped adaptor -- and used to leave a recording wedged: still shown as recording but no longer growing. Quill Radio now watches whether the file is growing at all, and treats four checks with no new bytes exactly like a dropped connection.
   - You should hear: A reconnection, or a stop that saves what was captured.

4. **Read a continuation correctly.** A continuation records only the remaining time to the original scheduled end -- a 60-minute show that drops at minute 50 records a ten-minute continuation, not another hour. And a part file keeps the original start timestamp in its name so the parts group together.
   - You should hear: The continuation's own length, spoken when it starts.
...
7. **Know which failures are worth retrying.** Only a genuinely terminal failure gives up: a full disk, or an HTTP 404, 410 or 451 that means the stream is truly gone. A network hiccup, a 5xx, or a momentary 403 from an expiring stream token is transient, and reconnects.
   - You should hear: Either a reconnection attempt, or a plain statement that the stream has gone.

Most of this happens without you. The reason to read it once is so that a part file, a short recording or a resume prompt is a thing you recognise rather than a thing you have to work out.

## More than radio

Podcasts, audiobooks, YouTube, television and the ACB Media schedule all arrive through the same tree and play with the same keys.

### Follow a podcast

Find a show, subscribe to it, find what is unheard, read a transcript without playing anything, and know where the line between Quill Radio and QUILL Cast falls.

*9 steps, about 8 minutes.*

1. **Open a podcast directory.** Two branches of the browse tree list shows: Podcasts (Apple), which is a country's storefront plus Apple's whole genre tree, and Podcast Index, which is the open directory. Neither needs a key, an account or a sign-in at any step.
   - Keys: Ctrl+B
   - You should hear: The branch, then its countries or categories.

2. **Look at a show without committing to it.** Open a show and its episodes are simply there: play one, favorite it, download it, read its transcript. You do not have to subscribe to listen, which is how you decide whether to.
   - Keys: Right arrow, Enter
   - You should hear: The episodes, newest first.
...
6. **Read an episode instead of playing it.** An episode whose feed publishes a transcript says transcript available on its row, and View Transcript opens it in the reader without playing anything. That is the fastest way to check whether an hour-long episode is about what you hoped.
   - Keys: Shift+F10
   - You should hear: The transcript window, with the episode's title at the top.

7. **Do the housekeeping.** A show's menu carries Move to Folder, Mark All as Played, Download All Episodes and Remove All Downloads. Each dims with its own reason when there is nothing to do -- nothing to mark, all 63 episodes are already played.
   - Keys: Shift+F10
   - You should hear: The action's confirmation, and the badges clearing.

8. **Hand an episode to QUILL Cast.** Play Next in QUILL Cast, Add to QUILL Cast Queue and Send to the QUILL Cast Inbox are a handoff rather than an instant change: Radio notes what you asked for and Cast carries it out the next time it opens. The confirmation says so.
   - Keys: Shift+F10
   - You should hear: It will be next in the QUILL Cast queue -- the future tense is deliberate.

9. **Know where the line is.** Playback, downloads and per-episode actions work here exactly as anywhere else in the tree. The rich side of podcasting -- automatic downloads, retention, the play queue, the full archive -- is QUILL Cast's job, and a serious podcast habit belongs there.
   - You should hear: Nothing: this is the fact that saves you looking for a setting that is not here.

Your place in an episode follows you between the two apps, and so do your bookmarks. Neither needs an account or a sync service.

Next: Audiobooks, archives and free music.

### Audiobooks, archives and free music

Find a book, keep your place in it, download a whole one while you go on listening, and understand exactly which sources will let you save what -- and why the ones that will not, will not.

*8 steps, about 8 minutes.*

1. **Find a book three ways.** LibriVox offers Recently Added, By Genre and By Author across some seven thousand readers. Project Gutenberg Audiobooks holds the 1,124 records that carry human-read audio. The Internet Archive holds Old Time Radio, the Live Music Archive, radio programmes and more.
   - Keys: Ctrl+B
   - You should hear: The branch's own shelves.
   - Worth knowing: There is deliberately no By Title in LibriVox: its catalogue supports author, genre and date filters and no title filter in any form, and a branch that quietly finds nothing is worse than one that is not offered.

2. **Open a book and play a chapter.** A book with chapters is a folder of chapters; a book that is one single reading is simply playable. Enter plays, and every transport key works -- including speed, chapters and Where Am I, because unlike a live station a recording has a timeline.
   - Keys: Right arrow, Enter
...
6. **Play a downloaded book as a book.** Its chapters are in proper order -- chapter 2 before chapter 10 -- and when one finishes the next starts on its own, announcing where you are: 4 of 40. When the last chapter ends, Quill Radio says so rather than simply going quiet.
   - You should hear: The next chapter starting, with its number and the total.

7. **Learn what cannot be saved, and why.** Quill Radio offers Download only where the source's terms clearly allow it, and asking anyway tells you which of four reasons applies: a live station has no file to save (that is what recording is for), Spotify is copy-protected, YouTube is a deliberate exclusion, and for Audius the choice belongs to the artist and is not stated in the listing.
   - Keys: Shift+F10
   - You should hear: The reason, named specifically rather than a general refusal.

8. **Find music that is genuinely free.** Audius gives you trending overall and within 27 genres, and drops pay-gated tracks rather than listing them and refusing. ccMixter is Creative Commons by tag, each track's licence on its own row. A Creative Commons track you save is written with its licence in a text file beside it.
   - You should hear: The track, and its licence spoken as part of the row.
   - Worth knowing: Mixcloud is metadata only: Quill Radio never extracts a Mixcloud stream, so activating a show opens it in your browser -- and the row says so before you press Enter.

Books, archives and free music behave like every other row in the tree. The only thing that changes source to source is what you are allowed to keep, and the app says which every time.

Next: YouTube, with no account anywhere.

### YouTube, with no account anywhere

Turn a link, a playlist or a whole channel into rows you can play, favorite and record -- and know honestly what this cannot do.
...
2. **Answer the one-time question.** The first time anything YouTube is added or played, Quill Radio asks once whether it may contact YouTube at all, and remembers the answer. It asks because a scheduled recording firing while nobody is watching should not be the first time the app ever touched YouTube.
   - You should hear: A one-time consent notice, with the rights position stated.

3. **Add a playlist as a list.** Add from YouTube Playlist lists the videos in the uploader's own order -- never re-sorted, because a series is meant to be worked through in order -- with each row reading as a sentence: position, title, length, publisher. Add Selected or Add All.
   - Keys: Ctrl+Shift+Y
...
A YouTube row plays, favorites, records and schedules like a station. What is saved is the page address, never a stream address, which is why a recording you book today still works next week.

### Watch television

Find a channel by country, category or postcode, play it with the same keys as everything else, and give yourself a programme guide if you want one.

*8 steps, about 6 minutes.*

1. **Find the branch.** Television sits in Browse Stations just above YouTube. It is built on the iptv.org community catalog -- roughly 9,300 playable channels after Quill Radio's own filtering, which removes adult-flagged channels, closed ones, ones with no stream, and streams that would fail the moment you pressed Enter.
   - Keys: Ctrl+B
   - You should hear: Television, then By Country and By Category.

2. **Open your own country.** A country whose feeds declare local coverage opens into Nationwide plus its states, and a state's list carries its own channels and its cities', each city named on the row. A country without local data stays a single list rather than making you open folders with nothing in them.
   - Keys: Right arrow
   - You should hear: The country, then Nationwide and the states.

3. **Search by place, not only by name.** Anywhere you can search -- Find Stations, Search All Sources, the Find box -- television answers by channel name, network, country, city, state, or a five-digit ZIP code. Typing 66044 answers with Kansas television, because a ZIP is a place.
   - Keys: Ctrl+F
   - You should hear: The matching channels.
   - Worth knowing: The ZIP mapping is by postal prefix. It is exact enough to narrow a list and claims nothing about what your antenna can pull in -- which is what the antennaweb link is for.

4. **Play a channel.** Enter plays it, and the video opens with the same captions, audio-track selection and transport every stream gets. Show or hide the picture with Ctrl+Shift+V -- the audio carries on either way, which is the point on a screen you are not looking at.
   - Keys: Enter, Ctrl+Shift+V
   - You should hear: The channel playing, and the video window announced when it opens.

5. **Turn on captions and read them.** Ctrl+Shift+K turns captions on and they open in their own window as text you can arrow through: each line joins the ones already spoken, and the line being spoken now is marked. It never announces itself, so read it whenever you like.
   - Keys: Ctrl+Shift+K
   - You should hear: The captions window, and then nothing until you go and read it.
   - Worth knowing: Follow Playback can be switched off so the window holds still while you read back. Escape closes it, and closing it turns captions off.

6. **Choose the audio track.** Ctrl+Shift+A lists the audio and described-audio tracks, leading with the language you read the app in, then the video's own original track, then the rest alphabetically -- so a channel with two dozen dubs is a list you can find your way down.
   - Keys: Ctrl+Shift+A
   - You should hear: The track list, your language first.

7. **Give yourself a programme guide.** Drop an XMLTV guide named tv_guide.xml into your Quill Radio data folder and every channel it covers gains a Now and Next line in its details. It is read locally, works offline, is re-read when you replace it, and is never fetched from anywhere. Delete the file and the lines disappear.
   - You should hear: Now, and Next, in the details of a covered channel.
   - Worth knowing: There is no one TV guide feed for the world -- guides are published per country and per provider -- so the file is deliberately yours to choose rather than the app's to pick.

8. **Keep the channel list current.** The channel list updates itself weekly; it is the largest catalog in the app at about 28 MB. Update the channel list now, at the top of the branch, fetches today's copy on demand and says what it is doing while it works.
   - You should hear: Progress while it fetches, then how many channels it has.

Television is video in a radio app, and it behaves like radio: favorite it, record it, schedule it, and drive it with the keys you already know.

### The ACB Media schedule

Read a published schedule across ten channels, find a programme, record it or be reminded about it, and understand why the list sometimes has nothing in it for today.

*9 steps, about 7 minutes.*
...
2. **Read the line above the list.** It always says how far the published schedule runs -- 49 programmes published; the published schedule runs 1 August to 15 August -- and says so plainly when that is behind us. ACB publishes a fortnight at a time and then stops, so for much of a month there is nothing posted for today. That is not a fault.
   - Keys: Shift+Tab
   - You should hear: The sentence, in a field you can arrow through word by word.

3. **Check whose clock the times are on.** ACB publishes in US Central time and Quill Radio converts every programme to your own clock. The line says so when the two differ -- because shown a bare 7:00 AM you have no way to tell a correct conversion from a missing one.
   - You should hear: Times are shown in your zone, and the zone ACB publishes in.

4. **Find one programme.** Three filters, all of which narrow what is listed and change nothing about what is playing. Search wants every word to appear somewhere, in any field, so blues tuesday finds the Tuesday blues show. Date jumps to a day that actually has programmes. Channel narrows to one of the ten.
   - Keys: Alt+S
   - You should hear: The number of programmes left after the filter.

5. **Do something with a programme.** Six verbs, each reachable from the context menu, from the buttons in the same order, and -- for Play -- from Enter: Play, Record, Remind Me, Add to Queue, Copy Details and Show Notes. A verb that cannot run is dimmed and says why.
   - Keys: Shift+F10
...
7. **Ask what is on without opening anything.** What Is On Now answers in one sentence across all ten channels. It answers from the stored schedule so it answers straight away -- a key that spends four seconds on a feed before speaking is a key nobody presses twice.
   - Keys: Ctrl+Alt+H
   - You should hear: What is on, across the channels, in one sentence.
...
Find your local weather transmitter by state, by call sign or by county, and know that the whole directory works with no connection.

*5 steps, about 4 minutes.*

1. **Open the branch.** Weather / NOAA in the browse tree is the real NOAA Weather Radio directory, state by state, each state announcing its transmitter count. The complete directory -- 1,035 transmitters -- is bundled inside the app, so this branch works offline.
   - Keys: Ctrl+B
   - You should hear: The states, each with its count.

2. **Find your transmitter.** Open a state and you get its actual transmitters, named with call sign, frequency and place -- KHB36 162.550 MHz Manassas. Enter plays the best available internet re-stream of it.
   - Keys: Right arrow, Enter
   - You should hear: The transmitter's call sign, frequency and place, then the audio.

3. **Search for it instead.** Weather radio is searchable by call sign, by SAME code, or by County, ST -- which is usually faster than arrowing a state with forty transmitters in it, and is how you find the one that actually covers you.
   - Keys: Ctrl+F
   - You should hear: The matching transmitters.

4. **Keep it where you can reach it.** Favorite your transmitter and put it in a folder with your local news station. In severe weather the thing you want is one keystroke, not a directory.
   - Keys: Ctrl+Shift+F
   - You should hear: Added, and the transmitter's name.

5. **Know where the rest of weather went.** Forecasts, alerts and background alert monitoring are Quill Weather's job now -- a separate app in the same family, opened from the QuillVille menu. What stays here is the radio part of weather, which is this branch.
   - You should hear: Nothing: this is the answer to where did the Weather menu go.
...
3. **Understand what retiring a pick does.** A pick that is retired vanishes from the picker and nothing you already added is touched. Your favorite stays, your subscription stays. A catalogue that could reach into your library would be one worth refusing to fetch at all.
   - You should hear: Nothing: this is the promise behind the window.
...
Set up Spotify search and browsing with your own Client ID, and understand precisely what a free account can and cannot do here before you spend the ten minutes.

*8 steps, about 9 minutes.*

1. **Read what a free account gets first.** On a free account you can search Spotify from inside Quill Radio and browse your saved shows, episodes, tracks and playlists. What you cannot do is have audio start inside Quill Radio -- Spotify does not license other applications to stream free-tier audio, and says so in its own developer documentation.
   - You should hear: Nothing: this step is the one that decides whether the rest is worth doing.
   - Worth knowing: This is about where the audio plays, not whether you may listen. The sensible use on a free account is to let Quill Radio do the finding -- the part that is genuinely awkward with a screen reader -- and play what you find in Spotify's own app.

2. **Create your own app identity.** Quill Radio ships no Spotify identity, so you supply your own and nothing of yours passes through anybody else's. Go to Spotify's developer dashboard, sign in with your ordinary account, and choose Create app. There is no charge and a free account works.
   - You should hear: Nothing from Quill Radio: this step happens in your browser.

3. **Fill in the app's details exactly.** Name and description are only for you. The Redirect URI must be http://127.0.0.1:43217/callback, character for character, including the port -- that address is how Spotify hands the finished sign-in back to your own computer. Tick Web API and Web Playback SDK.
   - You should hear: Nothing: this is still the browser.

4. **Copy the Client ID, and leave the secret alone.** Open your new app's settings and copy the Client ID. You will also see a Client secret: you do not need it and you should not paste it anywhere. Quill Radio signs in with the modern PKCE flow, which needs only the ID.
...
The parts you meet after the first week: what was that song, keeping a moment, sleeping, statistics, and where to look when something goes wrong.

### What was that song?

Four ways to answer it: ask out loud, open a copyable snapshot, look back through what has played, and read what the app can find out about the track.
...
Ask, review, copy, look back. The first is a key; the rest are for when a key spoken once is not enough.

Next: Keep a moment, and move around inside one.

### Keep a moment, and move around inside one

Mark where you are in one keystroke, get back to it later, and move through a recording by chapter without losing your place.
...
3. **Go back to one.** Enter goes to the highlighted bookmark. A recording, a video and a podcast episode all seek to the moment; a live station's bookmark tunes in now instead, because ten minutes into live radio meant ten minutes into your listening and tomorrow it means something else entirely.
   - Keys: Enter
   - You should hear: Either the position restored, or the station tuning in now.

4. **Open the chapter list.** Chapters works on more than a video's published marks: for a recording or a downloaded episode it reads the file's own chapter frames, and for an episode QUILL Cast has already analysed it reads the result Cast left in the shared cache. The list says which of those it is using in its first line.
   - Keys: Ctrl+Shift+C
...
6. **Move by chapter while it plays.** Next Chapter and Previous Chapter step through without opening the list, and they say where you landed. Like every bounded verb, they explain themselves on live radio rather than doing nothing.
   ...
   - You should hear: The chapter you moved to -- or why a live stream has none.

7. **Know that the list travels.** Your bookmarks are shared with QUILL Cast: one made here is in Cast's list and one made there is here, with no account and no sync service. A row this app cannot open still appears, with Go There dimmed and a reason -- hiding it would leave you wondering where your bookmark went.
   - You should hear: Nothing here: you will notice it the next time you open Cast.

One keystroke to keep a moment, one list to find it again, and a way to audition a chapter mark that costs you nothing.

Next: Sleeping, waking, and being left alone.

### Sleeping, waking, and being left alone

Set the radio to stop by itself, to start by itself, and to stop talking to you between certain hours -- and know precisely what each of those does not do.
...
6. **Recognise a reminder when it arrives.** The reminder sound comes first -- three rising bell tones, unlike anything else in the app -- and then the sentence. The sound is first on purpose: if you know it, you have already turned your attention by the time the words begin.
   - You should hear: Three rising tones, then what it is and when it starts.

7. **Keep the machine awake for it.** Keep the computer awake while playing or recording is on by default, so Windows does not sleep mid-listen. Your screen can still turn off, and the moment nothing is playing or recording the setting lets the computer sleep normally again.
   - Keys: Ctrl+,
   - You should hear: The setting read back.

Sleep, wake, quiet, remind. Four separate promises, each one honest about what it does not cover.

### Shape the sound

Per-station volume, a boost for a quiet stream, EQ and compression, pausing and rewinding live radio, and a playback speed that sticks.
...
5. **Choose a speed that stays chosen.** A speed you choose while a recording plays applies to every recording; a speed chosen on a YouTube row applies to YouTube rows. Per kind rather than per row, because being asked to set it again for each captured hour is the same feature with the cost moved onto you.
   ...
   - You should hear: The new speed, spoken as a number.

6. **Shorten the long pauses.** Skip Silence shortens the gaps in a recording, a YouTube row or an episode as it plays, taking effect on what is already playing with no interruption. It has no effect on live radio and says so if you turn it on while a station plays, rather than appearing to do nothing.
   - Keys: Ctrl+Shift+9
   - You should hear: Skip Silence on -- or the sentence explaining why a broadcast has no pauses to skip.

7. **Send the radio somewhere else.** Output Device routes the radio to a second sound card or a USB headset while your screen reader stays on the system default. Switching mid-song reconnects the station on the new device, which is where it matters.
   - Keys: Ctrl+Shift+D
   - You should hear: The device you chose, read back.

Per-station volume is the one to set up now. The rest are worth knowing about for the day a particular station annoys you.

Next: How much did I actually listen?.

### How much did I actually listen?

Read your listening broken down by station and by network, take it away as a spreadsheet, and understand exactly what is being counted.
...
3. **Know what counts.** Time counts only while audio is actually coming out. Connecting does not count. Buffering through dead air does not count. Paused does not count. The app sitting stopped overnight does not count.
   - You should hear: Nothing: this is why the number is lower than you expected, and right.

4. **Know what is deliberately not there.** Anything under ten seconds is not a session -- skipping past a station in a list is not listening, and a log full of three-second samples would make every per-station total meaningless. There is also no time saved by playing faster and no silence trimmed, because neither means anything for a broadcast.
...
It answers the question the recently-played list never could: not what did I have on, but how much.

Next: Back it up, move it, and keep it working.

### Back it up, move it, and keep it working

Take a copy of everything you have built, move it to another machine, stay up to date, and know what to do on the day something is genuinely wrong.
