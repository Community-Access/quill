...
- **QUILL Cast starts again.** On some builds QUILL Cast closed before its window ever appeared -- no error, no window, nothing to report. The cause was a single instruction telling Windows to expand the top of the library tree. That tree deliberately hides its invisible "root" node so the first thing you arrow to is a real folder or show rather than a meaningless container, and Windows refuses to expand a hidden node -- so it stopped the app instead. The instruction was never doing anything useful in the first place (a hidden root's children already *are* the visible top level), so it is now skipped, and a test keeps it that way.
- **Your braille display gets everything, and stops flickering.** Until now the standalone apps spoke but never wrote to braille: if you read by touch, QUILL Cast's confirmations, download progress, and errors simply were not there. Every announcement now reaches the display. And a burst of messages no longer flashes past faster than cell one can be read -- the first message of a quiet moment appears instantly, and anything arriving in the next fraction of a second settles to the newest rather than each one shoving the last aside. Errors are exempt: they always write through immediately, and can be held on the display instead of flashing by. You can choose whether braille carries the same wording as speech or a compact position-first form, and how long an identical repeated message is suppressed. These are shared accessibility settings, so they are set once in QUILL and apply here too.
- **Enter is now always the safe answer.** Four confirmations -- **Delete Folder**, **Delete Playlist**, **Remove All Episodes**, and **Delete Downloaded Files** -- used to open with Yes already selected, so pressing Enter out of habit destroyed the thing before you had finished reading the question. All four now start on No. Yes is still one arrow key away, but it has to be a decision. A check in the build fails if a destructive question is ever given a Yes default again.
- **Quillins, in QUILL Cast.** QUILL's small, sandboxed, permission-gated add-ons now run here too, from a **Quillins** menu of their own. An add-on says which apps it is written for, so you only ever see the ones meant for QUILL Cast. The bundled `cast-premium-auth` sample shows how one can supply the sign-in for a private, subscriber-only feed -- a companion to the built-in username and password support. Quillins are off in Safe Mode, and third-party ones stay disabled in this release.
- **Change any key, and reach the app from anywhere.** **Help > Keyboard Shortcuts...** opens the same searchable, conflict-aware Keyboard Manager QUILL uses, scoped to QUILL Cast's own commands. **Help > Global Hotkeys...** assigns system-wide keys -- and now includes **Show/Hide QUILL Cast to the Tray**, so the window tucks away or comes back with one keystroke even when another program has focus. Its default is Ctrl+Alt+Shift+Q; whichever Quill app starts first claims a shared chord, so change it here if you run more than one at once.
- **Spotify podcasts, taking shape.** Groundwork for playing Spotify-hosted shows has landed, hidden and off by default. See the section above for what it needs and what it cannot do.
...
- **Signed in everywhere that show goes -- and nowhere else.** Once a show has credentials, refresh, episode downloads, streaming playback, transcripts, and chapters all authenticate automatically. One deliberate security rule: credentials only ever travel to the feed's own host. Audio served from a third-party content network gets no password attached -- yours is never broadcast beyond the publisher who issued it.
- **Stored like a secret, because it is one.** Passwords live in Windows Credential Manager on an installed copy, or DPAPI-encrypted inside the portable `data` folder on a stick -- never in `podcasts.json`, never in logs, and never in an exported OPML file, so sharing your subscription list stays safe. One portable caveat, documented in the User Guide: DPAPI is tied to your Windows account and machine, so a stick moved to a new PC keeps every subscription but asks for private-feed passwords once more.
- **Honest errors.** A wrong or rotated password no longer hides behind "could not reach that feed" -- subscribing reopens the credentials prompt and says so, and a background refresh that fails to sign in announces exactly which show needs its credentials updated and where to do it.

- **Subscribing to a second show no longer means finding the list again.** In Add Podcast..., subscribing to a search result used to leave you standing on the Subscribe button with the results list somewhere behind you. Focus now lands back on the list, on the very row you just subscribed from, ready to arrow to the next show. It behaves the same whether the show subscribed, was already in your library, or failed. Adding by feed address is unchanged -- focus stays by the URL box, where you would want it.
- **Episodes, right there in the library tree.** Every show on the main page can now be expanded to reveal its episodes in place, so you never have to open the Manager just to reach one particular episode. Shows start collapsed, so the tree stays a list of shows rather than a wall of episodes. Enter on an episode plays it; Enter on a show still plays that show's next unplayed episode, exactly as before.
- **Keys for the things you do most.** The Episode menu gained real shortcuts: **Stop** on Ctrl+period, **Skip Back** and **Skip Forward** on Ctrl+Left and Ctrl+Right, and **Volume Down** and **Volume Up** on Ctrl+Down and Ctrl+Up -- the same volume keys Quill Radio uses, so the family behaves the same way. Play/Pause keeps Ctrl+P.
- **Alt+F4 can mean "get out of my way" instead of "quit".** A new Preferences checkbox (off by default) makes Alt+F4 send QUILL Cast to the system tray with the episode still playing. The titlebar X and the Exit command keep whatever you chose for closing the window, so the reflexive keyboard close stops ending your listening while a deliberate exit still exits.
- **Fixed: a show with no episodes.** Selecting a show in the Podcast Manager could show an empty episode list even when the show had plenty. Fixed.
- **Fixed: an update that hung.** Installing an update could stall behind a stray console window that appeared out of nowhere and stole your focus. That window is gone.
- **The confirming button is always OK.** Dialogs that used to confirm with Apply or Save now say OK, so the button that means "do it" is in the same place with the same name everywhere.
...
- Global hotkeys and keyboard shortcuts are now configured from QUILL Cast itself (Help > Global Hotkeys... and Help > Keyboard Shortcuts...); they no longer require opening full QUILL. The keymap is shared across the family, so a change here changes it everywhere.
- Accessibility settings that govern announcements -- braille on or off, braille style, the repeat window, sticky errors, interrupt severity -- are edited in QUILL's Preferences and honored here; QUILL Cast's own Preferences (Ctrl+,) covers only its four app-level switches.
