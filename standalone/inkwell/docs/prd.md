...
## 0. The application icon

**Resolved 2026-08-13.** Quill Inkwell has its own icon: a nib dipped into an
inkwell, on a terracotta tile, sharing the family's rounded-square shape and
amber accent. It is generated by `scripts/build_app_icons.py`, which owns the
design system for every app in the family, and
`tests/unit/scripts/test_app_icons.py` fails the build if any two apps ever
render the same face again.

For the record of what was wrong: the icon was a byte-identical copy of Quill
Weather's, which was itself a copy of Quill Radio's, carried over each time an
app was scaffolded from the last one. Inkwell is in `RELEASED_APPS`, so shipping
that would have put a second product on a listener's desktop wearing another
product's face -- confusing in the taskbar, in Alt+Tab, and in the tray, which
is exactly where a tray-resident app lives. It was the single hard blocker
before an Inkwell build could be cut, and it is no longer open.
