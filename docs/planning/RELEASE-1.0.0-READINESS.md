...
- [x] Complete **feature / command / surface inventory** generated from the codebase (718 commands, 647 dialog surfaces, ~290 feature sub-items, 3 menu bars).
- [x] **Sign-off pack under `docs/planning/signoff/`** — each item has **Works · Surface-exact · Accessible** boxes, grouped, counted, generated from the real registry (not memory):
  ...
  - `SIGNOFF-dialogs.md` (all 647 dialog surfaces) · `SIGNOFF-gated-apps.md` (44 gated-app commands to verify ABSENT).
  - `SIGNOFF-install-matrix.md` — portable vs system under all scenarios (E1–E6, all three portable signals), per app.
- [x] **Hand-held Acceptance Test Book under `docs/release/acceptance/`** — the guided companion to the inventory: 723 step-by-step scenarios (setup, exact keystrokes, projected outcome, per-scenario Pass/Fail/Blocked/N-A + Works/Surface-exact/Accessible sign-off) plus the 647-line dialog contract, covering all 645 core-editor command ids and the companion apps.
- [x] **Interactive runners** — `docs/release/acceptance/interactive/index.html` (the book) and `docs/planning/signoff/interactive/index.html` (the inventories): browser pages whose sign-off state saves as you go (localStorage + JSON export/import), with hide-completed / show-all and expand/collapse-all. Regenerate with `python scripts/gen_acceptance_html.py` / `python scripts/gen_signoff_html.py`.
- [ ] Human execution: check off every box across environments E1–E6 (this is the readiness gate). Start at `docs/release/acceptance/interactive/index.html`, in the run order it lists.
