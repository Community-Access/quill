...
| MENU-1 | Unified Preferences home | Menus | M | Done | Settings open through one tabbed front door (`Ctrl+,` then `General`) built from the shared settings registry. The three scattered Tools-menu toggles (Announcement Trace, Hey QUILL Commands, Watch Folder Monitoring) now redirect to Settings rather than toggling inline; their settings are controlled via registry-driven checkboxes in the Accessibility, Transcription, and Watch Folders tabs respectively. Menu items no longer mutate while menus are open. |
