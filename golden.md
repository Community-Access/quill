...
| OCR-4 | Accessible OCR review and insert surface | Image to text | M | In progress | The wx-free review renderer (`render_ocr_review`) emits the engine/language/low-confidence header and flags low-confidence lines (`OcrResult.low_confidence_lines`, `OcrLine.is_low_confidence`); tests cover the header, the low-confidence flag markers, and the text-only fallback. **Remaining (honest, Linux/cloud blocker):** the live stock read-only review dialog with insert/copy/discard action keys is not yet wired. The current `ocr_image_file` implementation directly inserts the recognized text into the editor without showing the review surface. The `render_ocr_review` function exists and is tested, but no wx dialog consumes it. **Windows finish-line:** Create an `OcrReviewDialog` (stock `wx.TextCtrl` with `TE_MULTILINE | TE_READONLY` + Insert/Copy/Discard buttons), populate it with `render_ocr_review(result)`, show it modally, and handle the three actions: Insert replaces editor content or inserts at cursor, Copy sends text to clipboard, Discard closes without action. Wire this dialog into `ocr_image_file` and any future clipboard/screen-region capture paths. Test on a real Windows machine, verify the dialog is screen-reader accessible and the three actions work as expected. |
...
| Tier 2 | Flagship experience | 58 | 45 | 13 | OCR-1, OCR-3, OCR-4, AGENT-1, AI-19, SET-2, SET-3, MENU-1, MENU-5, CTX-1, DICT-2, FEAT-19, DLG-1 |
...
| **1.0 subtotal** | Tiers 1, 2, 4, 6 (the QUILL 1.0 scope) | **144** | **82** | **62** | |
