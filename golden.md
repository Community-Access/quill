...
| OCR-1 | Native Windows OCR backend (zero-install) | Image to text | M | In progress | `quill/io/ocr.py` is now backend-pluggable (`OcrBackend` Protocol, `WindowsOcrBackend`, `TesseractBackend`, shared `OcrResult`/`OcrLine` with per-line confidence) with `select_engine`/`available_engines` and a clean unavailable path; the WinRT backend lives in `quill/platform/windows/windows_ocr.py` (`Windows.Media.Ocr`, offline, no network). Tests cover backend selection, fall-back, the unavailable path, and Tesseract TSV per-line confidence parsing. **Runtime blocker (Linux/cloud):** the native Windows OCR engine (`Windows.Media.Ocr` WinRT API) requires Windows 10+ and cannot be imported or exercised on Linux. The architecture, backend protocol, and fallback are complete and tested; the actual image→text recognition call (`BitmapDecoder`→`OcrEngine.RecognizeAsync`) is unverifiable in this environment (winsdk not installed). Windows finish line: manually verify the WinRT happy path on a Windows 10+ device (load an image, call the Windows backend, receive structured `OcrResult`, render the text) and add an integration test that exercises the full native-backend recognition under a Windows-only pytest marker. This row stays In progress. |
...
| OCR-3 | Capture sources: file, clipboard, screen region | Image to text | M | In progress | OCR from an image file is wired (`main_frame.ocr_image_file`, QUILL key + Tools menu, shared progress/outcome grammar, cancel path); the engine layer is capture-source-agnostic. **Runtime blocker (Linux/cloud):** clipboard-image capture and keyboard-driven screen-region capture both require a real windowing system with clipboard access and a display server, neither available in a cloud Linux environment. Clipboard capture needs `wx.Clipboard` with `wx.BitmapDataObject` and a working OCR engine to process the grabbed bitmap; screen-region capture needs coordinate input, a screenshot mechanism (`wx.ScreenDC`), and a working OCR engine to process the selected region. Windows finish line: implement the clipboard-capture command (`main_frame.ocr_from_clipboard`, read bitmap from clipboard, hand to engine, review result), implement the screen-region-capture command (`main_frame.ocr_screen_region`, keyboard-driven coordinate picker, screen grab, hand to engine, review result), and manually verify both capture paths on a Windows device with the native OCR backend. This row stays In progress. |
| OCR-4 | Accessible OCR review and insert surface | Image to text | M | In progress | The wx-free review renderer (`render_ocr_review`) emits the engine/language/low-confidence header and flags low-confidence lines (`OcrResult.low_confidence_lines`, `OcrLine.is_low_confidence`); tests cover the header, the low-confidence flag markers, and the text-only fallback. **Runtime blocker (Linux/cloud):** the live `OcrReviewDialog` (`quill/ui/ocr_review_dialog.py`) exists and is structurally complete (`wx.TE_MULTILINE | TE_READONLY` TextCtrl for the result text, Insert/Copy/Discard buttons, accessible names, focus handling), but requires `wx` import and a real windowing system to verify keyboard navigation, button actions, and screen-reader announcements. The dialog is not importable or exercisable on Linux without a running wx mainloop. Windows finish line: manually verify the OCR review dialog on a Windows device (open the dialog with a real OCR result, tab through controls, hear screen-reader announcements, press Insert to place text in editor, press Copy to clipboard, press Discard to close without action) and add a source-contract test asserting the dialog wiring (OcrReviewDialog import, button handlers, TextCtrl setup). This row stays In progress. |
...
| SET-2 | Tunable timing and pacing | Settings | M | In progress | QUILL key timeout, Quick Nav debounce and threshold, autosave interval, announcement throttle, Read Aloud rate, pitch, and sentence pause, and dictation sensitivity are now registry-backed `Settings` fields (clamped in `from_dict`) and auto-render in the tabbed dialog with per-setting reset. Live consumers now wired: Read Aloud rate/pitch drive the engine live; the **autosave interval** is consumed on startup and re-applied when the Settings dialog closes; the **announcement throttle** is enforced in `_set_status` (suppresses repeat status speech within the window); **Quick Nav debounce** (`quick_nav_debounce_ms`) defers type-ahead filtering via `wx.CallLater` and **Quick Nav minimum characters** (`quick_nav_min_chars`) hold off query matching until the field reaches the configured length; the **QUILL key timeout** is read by `_quill_key_timeout`; the **Read Aloud sentence pause** (`read_aloud_sentence_pause_ms`) inserts an interruptible gap between sentences across every TTS engine loop. **Runtime blocker (honest):** the `dictation_sensitivity` field is surfaced and stored but has no live consumer — the current dictation backend (`quill/platform/windows/dictation.py`) launches the Windows Win+H system recognizer via `keybd_event`, which exposes no programmatic sensitivity/threshold parameter. A sensitivity-aware live-mic capture engine (Whisper/Vosk with adjustable VAD threshold) must be implemented on Windows before this field can be wired. Windows finish line: build or integrate a local mic-capture backend that honors the stored sensitivity value and verify the wired consumer on a Windows device. This row stays In progress. |
| SET-3 | Tunable verbosity and announcements | Settings | M | In progress | A global `announcement_verbosity` level (minimal/normal/verbose) plus per-event overrides (`announce_wrap`, `announce_counts`, `announce_mode_changes`, `announce_spelling`, `announce_punctuation_level`) are registry-backed and surface in the Accessibility tab. Live consumers now wired: `announce_wrap` gates the " (wrapped)" suffix on Find/Replace; `announce_counts` decides whether the word-count summary is spoken or set quietly; `announce_mode_changes` gates QUILL key/browse mode enter/exit speech in `_quill_feedback` (sound still plays); `announce_spelling` short-circuits the passive live-misspelling hint. **Runtime blocker (honest):** the `announce_punctuation_level` field (none/some/all) is stored and surfaced in the Accessibility tab but has no live consumer — the current Read Aloud engine (`quill/platform/windows/read_aloud.py` using SAPI5/Windows.Media.SpeechSynthesis) accepts no punctuation-verbosity parameter, and the speech engines report no capability to control intonation vs. reading punctuation names. A Read Aloud backend extension (either a platform TTS API with punctuation control or a preprocessing layer that injects/strips punctuation tokens before synthesis) must be built before this setting can be wired. Windows finish line: extend the Read Aloud engine to honor the stored punctuation level (possibly via SSML, a different engine, or token preprocessing) and verify the consumer speaks punctuation as configured on a Windows device. This row stays In progress. |
...
| AGENT-1 | Accessibility agent (make this document accessible) | AI | L | In progress | A consented, announced, reversible plan audits and fixes structure, alt text, link text, and plain language, then re-audits and reports; every step is a reviewable diff and a single undo. The deterministic engine landed in `quill/core/accessibility_agent.py` (wx-free, strict-typed) on top of the in-repo `glow.py` + `plain_language.py` primitives: `build_plan` audits structure (markdown heading spacing, heading-jump, html missing `lang`), alt text, link text, and plain language and returns an ordered plan of per-step-consented `AgentStep`s; `apply_plan` applies only the accepted auto-fixable steps and re-audits, returning a run report. The `AccessibilityAgentDialog` (Tools > AI > Make This Document Accessible..., command `tools.ai_accessibility_agent`) shows each step as an accessible `wx.CheckListBox` with a before/after details pane; auto-fixable steps are pre-checked, advisory steps are listed for review. Applying replaces the whole editor buffer as **one undo** and opens a re-audit report tab; the summary is announced. **Advisory-only by design (honest):** structure and plain language steps are auto-fixed (heading-jump repair, `lang` attribute addition, passive-to-active rewording), but **alt-text and link-text steps are advisory-only** — the agent flags images lacking alt text and generic link text ("click here") but does not auto-write replacements, because meaningful alt text and link descriptions require human authoring judgment the deterministic engine cannot make. This is an intentional architecture decision: auto-fixing structure is safe and reversible, but auto-generating alt text or link summaries risks inserting nonsense that a screen-reader user would hear as fact. What "Done" means: AGENT-1 closes when the agent auto-fixes every mechanically-safe accessibility issue (structure, plain language, heading hierarchy) and flags every judgment-call issue (missing alt, generic link text) for human review, and the full flow (build plan, review, apply, re-audit, report) is manually verified on Windows with a real screen reader confirming that every announcement is intelligible and the undo is truly single-step. The alt-text and link-text **authoring** assist (prompting an AI backend to draft suggestions the user reviews and accepts one by one) is a separate, post-1.0 enhancement tracked under the broader "AI-assisted content generation" workstream. This row stays In progress until the mechanical-fix scope is verified complete on Windows. |
...
| AI-19 | Accessible subscription sign-in (no pasted API key) | AI | M | In progress | Derived from the Pi research (see `pi.md`). Add a guided, fully keyboard-and-screen-reader login that lets a user authenticate with an existing provider subscription via an interactive flow (for example OAuth device flow) instead of pasting an unreadable API key, alongside the current key path. The most user-facing accessibility win from Pi: "sign in with the subscription you already have" rather than "paste a 51-character secret you cannot see." Credentials in DPAPI, explicit consent, no silent network, registered in the GATE-9 egress audit. Sits behind the AIBackend boundary (AI-13) and reuses the connection error taxonomy (AI-17). **Delivered (strict-typed, fully tested):** `quill/core/ai/device_login.py` is a complete, wx-free OAuth 2.0 Device Authorization Grant (RFC 8628) state machine. `request_device_code` starts the flow; `announce_device_code` speaks the screen-reader instruction ("open <url> in your browser and enter the code WDJB-MJHT; this code expires in about 15 minutes."); `poll_once` classifies the provider's reply into pending / slow_down / authorized / denied / expired / error; `run_device_login` drives the full polling loop honoring the provider `interval`, backing off on `slow_down`, and stopping at `expires_in`, returning the terminal result; `describe_login_result` speaks the outcome. Every network exchange is an **injected** `poster`, so the engine adds no new `urlopen`/`urlretrieve` site to the GATE-9 egress inventory and is fully tested without a live endpoint (7 tests covering parse, state classification, success-after-pending, slow-down back-off, deadline expiry, denial, and the spoken announcements). ruff + strict mypy clean. **Runtime blocker (network-dependent):** the state machine and announcement logic are complete and tested with injected mock posters, but the **live HTTPS poster** (using the verified TLS context from SEC-5 and honoring the GATE-9 consent gate), the **DPAPI token storage** integration (via `quill/platform/windows/credential_manager.py`), the **consent surface** (a wx dialog explaining the OAuth flow and asking permission before the first network call), and the **AIBackend (AI-13) wiring** (so a provider like Azure OpenAI or Anthropic can offer "Sign in with your subscription" alongside the manual key path) are not yet built, so no live subscription sign-in actually ships. The unverifiable gap is the real network path: a cloud Linux environment cannot exercise a live OAuth device flow against a real provider endpoint (Azure AD, Anthropic, etc.) because the provider needs to see a real browser-based user login and device-code entry, and that requires a human user, a browser, and a network round-trip the agent cannot perform. What "Done" means: AI-19 closes when the HTTPS poster is wired (reusing SEC-5 TLS + GATE-9 consent), the DPAPI token store/retrieve is wired (persisting the access/refresh tokens), the consent dialog is built and shown before the first `request_device_code` call, at least one provider (Azure OpenAI or Anthropic) is wired to offer device-flow sign-in as an alternative to manual key entry in the AI Providers setup, and the full flow (user clicks "Sign in with subscription," dialog explains and asks consent, agent speaks the code and URL, user completes in browser, agent polls and speaks outcome, tokens are stored in DPAPI, generation succeeds with stored tokens) is manually verified end-to-end on a Windows device against a real provider with a real subscription. This row stays In progress. |
