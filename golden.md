...
| OCR-4 | Accessible OCR review and insert surface | Image to text | M | Done | The wx-free review renderer (`render_ocr_review`) emits the engine/language/low-confidence header and flags low-confidence lines (`OcrResult.low_confidence_lines`, `OcrLine.is_low_confidence`). The live review dialog (`OcrReviewDialog`, wired in `main_frame.ocr_image_file`) displays recognized text in a stock `wx.TextCtrl(... TE_MULTILINE | TE_READONLY)` with Insert/Copy/Discard actions; focus returns to the editor on close. Source-contract tests validate the wiring, the readonly text control, the three action buttons, and the modal accessibility hooks. The dialog is tracked in `dialogs.md`. |
