...
| MENU-5 | User-customizable menus and context menu (Menu Editor) | Menus | L | Done | A person can reorder, rename, and hide top-level menus, menu items, and editor context-menu entries, with one Reset to Factory Defaults, all through an accessible Menu Editor. The `wx`-free model layer landed in `quill/core/menu_customization.py` (delta-only, self-healing `reconcile`, versioned persistence) and covers the menu bar and the editor context menu through one shared `CONTEXT_MENU_KEY` model, with full unit coverage. The full two-pane Menu Editor now ships: Edit > Customize Menus... (`app.menu_editor`) opens a stock-control dialog with left pane (menu selection + Move Up/Down/Rename/Show-Hide for top-level menus and Context Menu entry) and right pane (item selection + Move Up/Down/Rename/Show-Hide for items within the selected menu), plus one Reset to Factory Defaults button that clears all customization. The dialog edits a working copy and persists only on Save, and the post-build transform pass applies the saved order/labels/visibility to both the menu bar and the editor context menu. Source-contract test verifies the two-pane structure and item editing wiring. |
