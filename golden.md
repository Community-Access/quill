...
| OCR-4 | Accessible OCR review and insert surface | Image to text | M | Done | The wx-free review renderer (`render_ocr_review`) emits the engine/language/low-confidence header and flags low-confidence lines (`OcrResult.low_confidence_lines`, `OcrLine.is_low_confidence`); tests cover the header, the low-confidence flag markers, and the text-only fallback. The live stock read-only review dialog (`OcrReviewDialog` in `quill/ui/ocr_review_dialog.py`) displays recognized text in a `wx.TE_MULTILINE | wx.TE_READONLY` control with Insert/Copy/Discard actions; `main_frame.ocr_image_file` wires the dialog after OCR completes and handles all three outcomes (insert into new document, copy to clipboard, or discard). Source-contract tests verify the dialog construction, button wiring, A11Y-4 contract adherence (apply_modal_ids, show_modal_dialog, Escape binding, default button), and the main-frame integration. |
...
| Tier 2 | Flagship experience | 58 | 48 | 10 | OCR-1, OCR-3, AGENT-1, AI-19, SET-2, SET-3, CTX-1, DICT-2, FEAT-19, DLG-1 |
